...
import com.ensea.nya.graphics.components.InvisibleClickableZone;
...
import static com.ensea.nya.graphics.gamesStates.animationEditor.AnimationEditor.*;
				...
				mouseY = frameOfAnim.input.getMouseY();
					if (!frameOfAnim.isDragging()) {
						...
						if (topSide)
							if (rightSide) {
								state = TOP_RIGHT;
								App.cursorResizeTOP_RIGHT();
							} else if (leftSide) {
								state = TOP_LEFT;
								App.cursorResizeTOP_LEFT();
							} else {
								state = TOP;
								App.cursorResizeTop();
							}
